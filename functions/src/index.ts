...
function generateItemsList(items: any[]): string {
...
function generateCustomerOrderEmail(order: any): string {
...
function generateAdminOrderEmail(order: any): string {
  const itemsList = generateItemsList(order.products);
...
function generateStatusChangeEmail(order: any, oldStatus: string): string {
  let statusMessage = '';
  let statusEmoji = '';
  let nextSteps = '';
